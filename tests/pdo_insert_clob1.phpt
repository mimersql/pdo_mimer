...
try {
    ...
    $clob = new Column('clob', [TYPE => 'CLOB(10M)'], [TYPE => PDO::PARAM_LOB, VALUES => [$fp]]);
    $db->createTables(new Table($table, [$id, $clob]));

    $stmt = $db->prepare("INSERT INTO $table ($id, $clob) VALUES (1, $clob->param)");
    $clob->bindValue($stmt)->execute();
    fclose($fp);
