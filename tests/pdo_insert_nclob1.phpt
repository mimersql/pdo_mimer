...
try {
    ...
    $nclob = new Column('nclob', [TYPE => 'NCLOB(10M)'], [TYPE => PDO::PARAM_LOB, VALUES => [$fp]]);
    $db->createTables(new Table($table, [$id, $nclob]));

    $stmt = $db->prepare("INSERT INTO $table ($id, $nclob) VALUES (1, $nclob->param)");
    $nclob->bindValue($stmt)->execute();
    fclose($fp);
